...
from .base import InternalDatasets, IOMode, LowLevelDataset
from ..meta import HDF5ExperimentalParameter, MetaHDF5Dataset

@enum.unique
class InternalPowderDatasets(enum.Enum):
    PowderIntensity = "/powder/intensity"
    PowderBaseline = "/powder/baseline"
    PowderPxRadius = "/powder/px_radius"  # for backwards compat
    PowderQ = "/powder/scattering_vector"  # for backwards compat

class LowLevelPowderDataset(LowLevelDataset):
    """
    Low-level interface to HDF5 for polycrystalline datasets
    ...
    def __init__(self, *args, **kwargs):
        ...
        required_dsets = [
            InternalPowderDatasets.PowderIntensity,
            InternalPowderDatasets.PowderBaseline,
            InternalPowderDatasets.PowderPxRadius,
            InternalPowderDatasets.PowderQ,
        ]
        if not all((dset.value in self for dset in required_dsets)):
            raise TypeError("")
    
    @classmethod
    def from_collection(cls, *args, **kwargs):
        return cls.from_dataset(dataset = super().from_collection(*args, **kwargs))

    @classmethod
    def from_dataset(cls, dataset, normalized=True, angular_bounds=None, callback=None):
        """
        Transform a LowLevelDataset instance into a LowLevelPowderDataset. This requires
        computing the azimuthal averages and associated quantities.

        Parameters
        ----------
        dataset : LowLevelDataset
            LowLevelDataset instance.
        ...
        Returns
        -------
        powder : LowLevelPowderDataset
        """
        fname = dataset.filename
        dataset.close()

        powder_dataset = cls(filename=fname, mode=IOMode.ReadWrite)

        resolution = tuple(powder_dataset.get_dataset(InternalPowderDatasets.Intensity)[0:2])
        maxshape = (len(powder_dataset.time_points), sqrt(2 * max(resolution) ** 2))
        for dset in [InternalPowderDatasets.PowderIntensity, InternalPowderDatasets.PowderBaseline]:
            powder_dataset.get_dataset(dset).create_dataset(
                name=dset.value,
                shape=maxshape,
                maxshape=maxshape,
                dtype=np.float,
                fillvalue=0.0,
                **powder_dataset.compression_params,
            )

        # Radius from center in units of pixels
        shape = powder_dataset.get_dataset(InternalPowderDatasets.PowderIntensity).shape
        placeholder = np.arange(0, shape[-1])
        for dset in [InternalPowderDatasets.PowderPxRadius, InternalPowderDatasets.PowderQ]:
            powder_dataset.get_dataset(dset).create_dataset(
                name=dset.value,
                data=placeholder,
                maxshape=(maxshape[-1],),
        ...
        powder_dataset._compute_angular_averages(
            normalized=normalized, angular_bounds=angular_bounds, callback=callback
        )
    ...
    def px_radius(self):
        ...
        return np.array(self.get_dataset(InternalPowderDatasets.PowderPxRadius))

    @property
    def scattering_vector(self):
        """ Array of scattering vector norm :math:`|q|` [:math:`1/\\AA`] """
        return np.array(self.get_dataset(InternalPowderDatasets.PowderQ))
    ...
    def powder_calq(self, crystal, peak_indices, miller_indices):
        ...
        self.get_dataset(InternalPowderDatasets.PowderQ).resize(I.shape)
        self.get_dataset(InternalPowderDatasets.PowderQ).write_direct(q)

    # powder_eq is fast enough that we don't need to
    # store it in-file
    ...
    def powder_eq(self, bgr=False):
        ...
        b4t0_slice = self.get_dataset(InternalPowderDatasets.PowderIntensity)[:t0_index, :]
        ...
        bg = self.get_dataset(InternalPowderDatasets.PowderBaseline)[:t0_index, :]
        return np.mean(b4t0_slice - bg, axis=0)

    def powder_diffraction_pattern(
        self, timedelay, bgr=False, relative=False, out=None
    ):
        """
        ...
        I : ndarray, shape (N,) or (N,M)
            Diffracted intensity [counts]
        """
        dataset = self.get_dataset(InternalPowderDatasets.PowderIntensity)
    ...
    def powder_baseline(self, timedelay, out=None):
        ...
        """
        dataset = self.get_dataset(InternalPowderDatasets.PowderBaseline)
    ...
    def powder_time_series(
        ...
        trace = np.array(
            self.get_dataset(InternalPowderDatasets.PowderIntensity)[:, i_min:i_max]
        )
        if bgr:
            trace -= np.array(
                self.get_dataset(InternalPowderDatasets.PowderBaseline)[:, i_min:i_max]
            )
    ...
    def compute_baseline(self, first_stage, wavelet, max_iter=50, level=None, **kwargs):
        ...
        self.get_dataset(InternalPowderDatasets.PowderBaseline).resize(baseline.shape)
        self.get_dataset(InternalPowderDatasets.PowderBaseline).write_direct(baseline)
    ...
    def _compute_angular_averages(
        ...
        self.get_dataset(InternalPowderDatasets.PowderIntensity).resize(rintensity.shape)
        self.get_dataset(InternalPowderDatasets.PowderIntensity).write_direct(rintensity)

        self.get_dataset(InternalPowderDatasets.PowderPxRadius).resize(px_radius.shape)
        self.get_dataset(InternalPowderDatasets.PowderPxRadius).write_direct(px_radius)

        # Use px_radius as placeholder for scattering_vector until calibration
        self.get_dataset(InternalPowderDatasets.PowderQ).resize(px_radius.shape)
        self.get_dataset(InternalPowderDatasets.PowderQ).write_direct(px_radius)

        self.get_dataset(InternalPowderDatasets.PowderBaseline).resize(rintensity.shape)
        self.get_dataset(InternalPowderDatasets.PowderBaseline).write_direct(
            np.zeros_like(rintensity)
        )
